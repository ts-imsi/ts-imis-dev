...
(function(w){
	...
	w.Calendar = function (opt) {
		...
		tpl += '<div class="calendar-year" id="calendar-year"></div>';
		tpl += '<div class="calendar-month" id="calendar-month" ></div>';
	...
	function setContent(el, fistWeek, monDaynum, nowDay){
		...
		for(var i=1;i <= monDaynum;i++){
			...
			if(i==nowDay )
			{ 
			    var currentMonthPD=document.getElementById("calendar-month").innerHTML;
			    var currentYearPD=document.getElementById("calendar-year").innerHTML;
				var mon=now.getMonth()+1+"月";
				var yea=now.getFullYear()+"年";
			    if(currentMonthPD == mon && currentYearPD == yea)
				{
					subContent.classList.add("today");//判断当前日期，在日期上添加样式today
				}    
...
/********************************判断有签到情况的日期，添加样式chooseday，样式与canChoose平级***********************************************/	
