app.controller('CountReport', ['$scope', '$modal', '$http', '$filter','$log', function ($scope, $modal, $http,$filter, $log) {
    ...
    this.countReportByType=function(showType){
        ...
        $http.post("/ts-project/countReport/getCountReport",angular.toJson(param)).success(function (result) {
            if(result.success){
                selt.countRPro = result.object.tbOutputValueCounts;
                selt.totalCount=result.object.totalCount;
                selt.finishedCount=result.object.finishedCount;
                selt.unfinishedCount=result.object.unfinishedCount;
                selt.nextUnCount=result.object.nextUnCount;
            }else{
                selt.countRPro=[];
            }
